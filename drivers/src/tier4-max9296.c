...
int tier4_max9296_sdev_register(struct device *dev, struct gmsl_link_ctx *g_ctx)
{
	...
	for (i = 0; i < priv->num_src; i++) {
		if (g_ctx->serdes_csi_link ==
			priv->sources[i].g_ctx->serdes_csi_link) {
			dev_err(dev,"[%s] : Serdes CSIi link is in use\n", __func__);
			err = -EINVAL;
			goto error;
		}
		/*
		...
		struct gmsl_link_ctx *g_ctx)
{
	...
	for (i = 0; i < g_ctx->num_streams; i++) {
		...
		struct reg_pair map_pipe_yuv8[] = {					// for YUV8
			/* addr, val */
		...
		if (g_stream->st_data_type == GMSL_CSI_DT_YUV_8) {			// for YUV8
			...
			dev_info(dev,"[%s] : No mapping for GMSL_CSI_DT_UED_U1\n", __func__);
			continue;
		} else {
		...
		pipe_id = tier4_max9296_get_available_pipe(dev,
